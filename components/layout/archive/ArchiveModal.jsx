...
export default function ArchiveModal({data, setSingleReportData}) {
    ...
    return (
        <div className='w-full max-w-5xl px-16 pb-6 mx-auto pt-11 bg-lightgrey rounded-3xl'>
        ...
        <div className='flex items-center w-1/3 mt-12'>
          <div className="relative z-0 w-full group">
            <input
              ...
              placeholder=" "
            />
            <label htmlFor="search" className="absolute font-bold text-sm text-darkblue duration-300 transform -translate-y-6 scale-75 top-3 origin-[0] peer-focus:left-0 peer-focus:text-darkblue peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Such nach einem Begriff</label>
          </div>
        ...
        <div className={`mt-4 ${viewSetting == 'list'? "" : "grid grid-cols-3 gap-4"}`}>
          ...
          .map((report) => (
            <div className='modal-element' key={report.id} >
              {viewSetting === 'list' ? (
                <ReportListElement report={report} setData={setData} setSingleReportData={setSingleReportData}/>
              ) : (
                <ReportTileElement report={report} setData={setData} setSingleReportData={setSingleReportData}/>
              )}
            </div>
          ))}
        </div>
